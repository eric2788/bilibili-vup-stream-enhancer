...
test("嘗試使用 Cloudflare AI 對話", async () => {
  test.skip(
    ...
    "請設定 CF_ACCOUNT_ID 和 CF_API_TOKEN 環境變數"
  );
...
test("嘗試使用 OpenAI 對話", async () => {
  ...
  logger.info("正在测试 OpenAI json 返回请求...");
  const res = await llm.prompt("你好");
  ...
  logger.info("正在测试 OpenAI SSE 请求...");
  const res2 = llm.promptStream("地球为什么是圆的?");
...
test("嘗試使用 Web LLM 對話", async ({ modules, page }) => {
  ...
  const res = await page.evaluate(async () => {
    const { llms } = window as any;
    const llm = llms.createLLMProvider({
      provider: "webllm",
      module: "TinyLlama-1.1B-Chat-v1.0-q4f16_1-MLC-1k"
    });
