...
addMigrationTransfer('tongchuanMans', 'settings.features', 'jimaku.listingZone.tongchuanMans', (v) => v.map((id) => ({ id, name: `用戶${id}`, addedDate: new Date().toLocaleDateString()})))
addMigrationTransfer('tongchuanBlackList', 'settings.features', 'jimaku.listingZone.tongchuanBlackList', (v) => v.map((id) => ({ id, name: `用戶${id}`, addedDate: new Date().toLocaleDateString()})))
addMigrationMapping('subtitleColor', 'settings.features', 'jimaku.jimakuZone.color')
addMigrationTransfer('blacklistRooms', 'settings.listings', 'blackListRooms', (v) => v.map((room) => ({ room, addedDate: new Date().toLocaleDateString() })))
